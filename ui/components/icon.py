...
class Icon(QLabel):
    ...
    SEARCH_PATHS = [
        "assets/icon",   # Primary icon folder
        "assets/images",
        "assets/icons",
        "assets",
    ]
    ...
    def _find_icon_path(self) -> Optional[str]:
        ...
        for search_path in self.SEARCH_PATHS:
            for extension in self.EXTENSIONS:
                # Try exact name
                icon_path = os.path.join(search_path, f"{self.icon_name}{extension}")
                if os.path.exists(icon_path):
                    return icon_path

                # Try capitalized name (case variations)
                icon_path = os.path.join(search_path, f"{self.icon_name.capitalize()}{extension}")
                if os.path.exists(icon_path):
                    return icon_path

                # Try uppercase name
                icon_path = os.path.join(search_path, f"{self.icon_name.upper()}{extension}")
                if os.path.exists(icon_path):
                    return icon_path
    ...
    def load_qicon(icon_name: str, size: int = IconSize.MEDIUM.value) -> Optional[QIcon]:
        ...
        for search_path in Icon.SEARCH_PATHS:
            for extension in Icon.EXTENSIONS:
                icon_path = os.path.join(search_path, f"{icon_name}{extension}")
                if os.path.exists(icon_path):
                    return QIcon(icon_path)

                # Case variations
                icon_path = os.path.join(search_path, f"{icon_name.capitalize()}{extension}")
                if os.path.exists(icon_path):
                    return QIcon(icon_path)

                icon_path = os.path.join(search_path, f"{icon_name.upper()}{extension}")
                if os.path.exists(icon_path):
                    return QIcon(icon_path)
    ...
    def get_all_search_paths(icon_name: str) -> List[str]:
        ...
        for search_path in Icon.SEARCH_PATHS:
            for extension in Icon.EXTENSIONS:
                paths.append(os.path.join(search_path, f"{icon_name}{extension}"))
                paths.append(os.path.join(search_path, f"{icon_name.capitalize()}{extension}"))
                paths.append(os.path.join(search_path, f"{icon_name.upper()}{extension}"))
        return paths
